		...
		deps.forEach(function(required)
		{
			...
			q.add(function(nx)
			{
				try
				{
					...
					{
						_scanJSFile({ input: fullPath, output: org, tmpDir: tmpDir, model: model }, replaceDep);
					}
...
function _runTargetProject(inputFile, callback)
{
	...
	exec('mkdir -p '+ tmp+'; cd '+tmp+'; tar -xf '+ inputFile, function(err)
	{
		...
		//if not bundled, then link
		if(!config.slug.bundle)
		{
			console.success('Linking slug');
			exec('cd '+tmp+'; npm link', run);
		}
		else
		{
			run();
